...
module Telnyx
  class BrandTest < Test::Unit::TestCase
    ...
    should "get feedback" do
      ...
      assert_requested :get, "#{Telnyx.api_base}/10dlc/brand/123/feedback"
      assert brand_feedback.is_a?(Telnyx::TelnyxObject)
    end

    should "list external vettings" do
      external_vettings = Telnyx::Brand.list_external_vettings("123")
      assert_requested :get, "#{Telnyx.api_base}/10dlc/brand/123/externalVetting"
      assert external_vettings.is_a?(Telnyx::TelnyxObject)
    end

    should "order external vetting" do
      external_vetting = Telnyx::Brand.order_external_vetting("123", {})
      assert_requested :post, "#{Telnyx.api_base}/10dlc/brand/123/externalVetting"
      assert external_vetting.is_a?(Telnyx::TelnyxObject)
    end

    should "import external vetting record" do
      external_vetting_record = Telnyx::Brand.import_external_vetting_record("123", {})
      assert_requested :put, "#{Telnyx.api_base}/10dlc/brand/123/externalVetting"
      assert external_vetting_record.is_a?(Telnyx::TelnyxObject)
    end

    should "revet brand" do
      revet = Telnyx::Brand.revet("123")
      assert_requested :put, "#{Telnyx.api_base}/10dlc/brand/123/revet"
