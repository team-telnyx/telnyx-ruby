...
module Telnyx
  module Internal
    ...
    #
    # @example
    #   if default_flat_pagination.has_next?
    #     default_flat_pagination = default_flat_pagination.next_page
    #   end
    #
    # @example
    #   default_flat_pagination.auto_paging_each do |oauth_client|
    #     puts(oauth_client)
    #   end
        ...
        req = @req.merge(
          {
            query: @req.fetch(:query, {}).except(
              :"page[number]",
              :"page[number]".to_s
            ).merge("page[number]": (meta&.page_number || 1).to_i.succ)
          }
        )
        @client.request(req)
      end
      ...
      def initialize(client:, req:, headers:, page_data:)
        ...
        in {meta: Hash | nil => meta}
          @meta = Telnyx::Internal::Type::Converter.coerce(
            Telnyx::Internal::DefaultFlatPagination::Meta,
            meta
          )
        else
        end
